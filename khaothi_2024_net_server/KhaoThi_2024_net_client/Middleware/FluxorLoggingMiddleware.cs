...
namespace KhaoThi_2024_net_client.Middleware
{
    public class FluxorLoggingMiddleware : Fluxor.Middleware
    {
        ...
        public override async Task InitializeAsync(IDispatcher dispatcher, IStore store)
        {
            ...
            {
                await Logger.Error($"❌ [Fluxor] Lỗi trong InitializeAsync: {ex.Message}", ex);
            }
        }
        ...
        public override async void AfterInitializeAllMiddlewares()
        {
            ...
            {
                await Logger.Error($"❌ [Fluxor] Lỗi trong AfterInitializeAllMiddlewares: {ex.Message}", ex);
            }
        }
        ...
        public override async void BeforeDispatch(object action)
        {
            ...
            {
                await Logger.Error($"❌ [Fluxor] Lỗi trong BeforeDispatch cho action {action.GetType().Name}: {ex.Message}", ex);
            }
        }
        ...
        public override async void AfterDispatch(object action)
        {
            ...
            {
                await Logger.Error($"❌ [Fluxor] Lỗi trong AfterDispatch cho action {action.GetType().Name}: {ex.Message}", ex);
            }
        }
    }
}